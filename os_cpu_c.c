...
*
*                           (1) At each call site, the current size of the calling function뭩 stack frame is a multiple of 8 bytes.
...
{
	GLCD_DisplayChar(EnemyBulletLv2.y, EnemyBulletLv2.x, 0, ' ');
}

void DrawEnemyBulletLv2(void)
{
...
void moveEnemyBulletlv2(void) {

	if (EnemyBulletLv2.exist == 0)
		return;




	//	if(--EnemyBulletLv2.nStay==0){
	//	EnemyBulletLv2.nStay=EnemyBulletLv2.nFrame;

	EraseEnemyBulletLv2();

	if (EnemyBulletLv2.y > 38 || EnemyBulletLv2.x < 1)
		EnemyBulletLv2.exist = 0;
	else {
		if (rand() % 20) { //플레이어쪽으로 총알이 오도록 함
						   //			if (EnemyBulletLv2.x < px)
						   //				EnemyBulletLv2.x++;
						   //			else
			EnemyBulletLv2.x--;
		}
...
void  OSTimeTickHook(void)
{
	static int flag = 0;
	char buf[30];
	if (flag == 1) {//움직이고 총쏘는데 대각선으로

					//drawEnemy
		EnemyLv2.x = 25; EnemyLv2.y = 0;
		EnemyLv2.dest = -1;
		EnemyLv2.nFrame = 1;
		EnemyLv2.exist = 1;

		//EnemybulletAction lv2
		EnemyBulletLv2.x = EnemyLv2.x;
		EnemyBulletLv2.y = EnemyLv2.y + 1;
		EnemyBulletLv2.nFrame = EnemyBulletLv2.nStay = 8;
		EnemyBulletLv2.exist = 1;
	}
	...
	flag++;
	//enemyMoveAction
	if (EnemyLv2.x < 0) { //적군이 화면밖을 벗어날 경우
		EnemyLv2.exist = 0;
		sprintf(buf, "   ");
		GLCD_DisplayString(EnemyLv2.y, EnemyLv2.x, 0, buf);
	}
	else {
		sprintf(buf, "   ");
		GLCD_DisplayString(EnemyLv2.y, EnemyLv2.x - 3, 0, buf);

		EnemyLv2.x += EnemyLv2.dest;

		sprintf(buf, "%s", level2Enemy);
		GLCD_DisplayString(EnemyLv2.y, EnemyLv2.x - 3, 0, buf);
	}
